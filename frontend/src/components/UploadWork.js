...
const UploadWork = ({ onClose, fetchData }) => {
  ...
  const handleUploadWork = async (e) => {
    ...
    setData((preve) => {
      return {
        ...preve,
        workImage: [...preve.workImage, uploadImageCloudinary.url],
      };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(SummaryApi.uploadWork.url, {
      method: SummaryApi.uploadWork.method,
      credentials: "include",
      headers: {
      ...
      <div className="bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden">
        ...
        >
          ...
          <input
            type="text"
            id="workName"
            placeholder="ажлын нэрийг оруулна уу?"
            name="workName"
            value={data.workName}
          ...
          <div>
            {data?.workImage[0] ? (
              ...
              <p className="text-red-600 text-xs">*Ажлын зургаа оруулна уу?</p>
            )}
          </div>
          ...
          <input
            type="number"
            id="price"
            placeholder="ажлын үнэ оруулна уу"
            value={data.price}
            name="price"
            onChange={handleOnChange}
            className="p-2 bg-slate-100 border rounded"
          ...
          <input
            type="number"
            id="sellingPrice"
            placeholder="ажил хийгдэх нэгжийн үнэ оруул"
