...
const CategoryRecommendedWorkDisplay = ({ category, heading }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const categoryProduct = await fetchCategoryAllWork(category);
    setLoading(false);
                  ...
                  <div className="p-4 grid gap-3">
                    ...
                    <div className="flex gap-3">
                      ...
                      {/* <p className="text-slate-500 line-through">
                        {displayMNTCurrency(work?.price)}
                      </p> */}
                    </div>
                    <button
                      className="text-sm uppercase bg-red-600 hover:bg-red-700 text-white px-3 py-0.5 rounded-full"
...
export default CategoryRecommendedWorkDisplay;
